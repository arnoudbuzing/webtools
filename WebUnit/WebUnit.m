WebUnit`$CurrentWebSession;
WebUnit`Private`webDriver;
WebUnit`Private`webDriverBaseURL;

BeginPackage["WebUnit`", {"WebUnit`WebDriverAPI`"}]

InstallWebUnit::usage = "InstallWebUnit[] launches the default web driver which allows Mathematica to communicate with a web browser. InstallWebUnit[driver] launches the specified driver.";
...
(* where am i? *)
$WebUnitDirectory = DirectoryName[$InputFileName];
...
InstallWebUnit[] := InstallWebUnit["ChromeDriver"];

InstallWebUnit[driver_] := Module[{dir},
